class ColorGame {
    ...
    init(userOption) {
        ...
        document.getElementsByClassName(
            "wgt-score"
        )[0].innerHTML = `得分：<span id="score">${this.score}</span> 时间：<span id="timer">${this.time}</span>`;
    ...
    <div class="block-inner" style="background-color: #${normalColor}"></div>
        ...
        for (let i = 0; i < col * col; i++) {
            arr.push(item);
            console.log(
                '横:', (randomBlock + 1) % col === 0 ? (randomBlock + 1) / col : Math.floor((randomBlock + 1) / col) + 1,
                ';',
                '竖:', (randomBlock + 1) % col === 0 ? col : (randomBlock + 1) % col,
                ';',
                '总:', col
            );
        }
...
function randomColor(min, max) {
    let r = randomNum(min, max).toString(16);
    let g = randomNum(min, max).toString(16);
    let b = randomNum(min, max).toString(16);
    return r + g + b;
}
// 随机数
